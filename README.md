...
- **Teaching notebooks** — four Livebook tutorials in [`notebooks/`](./notebooks) build the Maude mental model from terms to the deployment gate, plus five stage livebooks in `priv/livebooks/`.
...
| [`/rules/new`](http://localhost:4000/rules/new) | rule creation with an in-request check |
...
```text
docs/talk/                 abstract, script, slides, Q&A, sources (CC BY-NC-ND)
...
docker/                    broker/simulator/Prometheus/Grafana support stack
...
`mix check` runs warning-free compilation and docs, formatting, strict Credo, Doctor, Sobelow, dependency and secret audits, coverage, Dialyzer, the OTP release, slide artifacts, and both Compose topologies. Focused lanes (property, E2E, stress, LLM) and the reasoning behind the coverage policy are described in [`docs/testing.md`](./docs/testing.md).
...
The **software** is MIT licensed ([LICENSE](./LICENSE)); run it, study it, build on it. The **talk material** under [`docs/talk/`](./docs/talk) — script, slides, narrative, Q&A — is [CC BY-NC-ND 4.0](./docs/talk/LICENSE.md): published for transparency, not for re-delivery. The separately installed Maude interpreter is GPL.
