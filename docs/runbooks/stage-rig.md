...
1. [`../talk/memorize.md`](../talk/memorize.md) contains the seven-beat story, twenty-six anchors, and protected lines. Drill those until they are yours (`make learn`).
...
The 26-slide sequence in `docs/talk/slides/deck.md` matches the live presenter and rehearsal sources. The Markdown source is not a separately built stage export.
...
Slide, panel, zoom, completed steps, and timer state live in the server. Browser refresh restores that state and the shared pane results. Versioned clock checkpoints also survive application restart, excluding time spent offline; malformed or older checkpoint schemas and checkpoints from another deck sequence start from slide 1. The 26-slide deck uses checkpoint version 3 and cannot restore the old 18-slide positions. In-memory pane results survive clock or action-worker restart, but a full application restart clears them. That is the recovery move: Cmd+R, not window juggling.
